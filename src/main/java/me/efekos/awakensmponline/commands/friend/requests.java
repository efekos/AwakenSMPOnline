...
public class requests extends SubCommand {
    ...
    public String getDescription() {
        return AwakenSMPOnline.getPlugin().getConfig().getString("messages.commands.main.desc-fr-req");
    }
    ...
    public void perform(CommandSender sender, String[] args) {
        ...
        for (Request request : RequestsJSON.getAllData()){
            if(request.getTo() == p.getUniqueId()){
                if(request.getType() == RequestType.FRIEND){
                    OfflinePlayer frP = p.getServer().getOfflinePlayer(request.getFrom());
                    p.spigot().sendMessage(new TextComponent(frP.getName() + " "),Friends.makeAcceptButton(request.getId()),Friends.makeDenyButton(request.getId()));
                }
            }
        }
        p.sendMessage("------------------");
